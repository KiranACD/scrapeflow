...
export function MobileSidebar() {
    ...
    return (
        <div className="block w-[50px] border-separate bg-secondary md:hidden">
            <nav className="w-full h-screen flex flex-col items-center py-2 gap-4">
                <Sheet open={isOpen} onOpenChange={setIsOpen}>
                    ...
                    <SheetContent className="w-[300px] space-y-4" side={"left"}>
                        <Logo />
                ...
                {routes.map(route => (
                    <Link key={route.href} href={route.href} className={buttonVariants({variant: "ghost"})}>
                        <route.icon size={20}/>
                    </Link>
                ))}
            </nav>
